...
let CreateOrderUseCase = class CreateOrderUseCase {
    ...
    paymentRepository;
    dataSource;
    constructor(orderRepository, orderItemRepository, customerRepository, productRepository, customerProductPriceRepository, stockMovementRepository, paymentRepository, dataSource) {
    ...
    async execute(dto, userId) {
        ...
        for (const itemDto of dto.items) {
            ...
            if (requestedUnitPrice <= 0) {
                throw new business_exception_1.InvalidOrderException(`Prix unitaire invalide pour le produit ${product.name}`);
            }
            const itemSubtotal = requestedUnitPrice * itemDto.quantity;
            subtotal += itemSubtotal;
            const orderItem = new order_item_entity_1.OrderItem('', itemDto.productId, product.name, itemDto.quantity, requestedUnitPrice);
            orderItems.push(orderItem);
            const stockMovement = new stock_movement_entity_1.StockMovement(itemDto.productId, stock_movement_entity_1.StockMovementType.VENTE, itemDto.quantity, requestedUnitPrice, undefined, userId);
            ...
            if (Number(requestedUnitPrice) !== Number(referencePrice)) {
                await this.customerProductPriceRepository.upsert(dto.customerId, itemDto.productId, requestedUnitPrice);
            }
        }
...
exports.CreateOrderUseCase = CreateOrderUseCase = __decorate([
    ...
    __param(4, (0, common_1.Inject)('ICustomerProductPriceRepository')),
    __param(5, (0, common_1.Inject)('IStockMovementRepository')),
    __param(6, (0, common_1.Inject)('IPaymentRepository')),
