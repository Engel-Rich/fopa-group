...
let RegisterUserUseCase = class RegisterUserUseCase {
    ...
    async execute(dto) {
        ...
        if (existingPhone) {
            throw new business_exception_1.AlreadyExistsException('Un utilisateur avec ce téléphone existe déjà');
        }
        const hashedPassword = await bcrypt.hash(dto.password, 10);
        const user = new user_entity_1.User(dto.name, hashedPassword, dto.username, dto.phone, dto.role, dto.email);
        ...
        const refreshSecret = this.configService.get('JWT_REFRESH_SECRET') || this.configService.get('JWT_SECRET') || 'your-refresh-secret-key';
